...
public sealed record AssingPoliciesToRoleCommand(Guid RoleId, AssingPoliciesToRoleInput Input) : ICommand<Result<RoleDetailsOutput>>;
...
internal class AssingPoliciesToRoleCommandHandler(
    ...
    ) : ICommandHandler<AssingPoliciesToRoleCommand, Result<RoleDetailsOutput>>
{
    public async Task<Result<RoleDetailsOutput>> Handle(AssingPoliciesToRoleCommand command, CancellationToken ct)
    {
        ...
        if (role is null)
            return Result<RoleDetailsOutput>.NotFound("Role not found");
        ...
        if (policies.Count < distinctPolicies.Count)
            return Result<RoleDetailsOutput>.Failure("Some policies id do not exist", ErrorType.Conflict);
        ...
        if (newPolicies.Count == 0)
            return Result<RoleDetailsOutput>.Failure("All policies are already assigned to this role", ErrorType.Conflict);

        var result = await roleRepository.AssignPoliciesToRoleAsync(role, policies, ct);

        return Result<RoleDetailsOutput>.Success(result.ToDetailsOutput());
    }
